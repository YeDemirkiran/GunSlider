...
{
    public static void ChooseTarget(ref AimTarget aimTarget, BodyHitpoints targetHitpoints)
    {
        Hitpoint target = targetHitpoints.hitpoints[Random.Range(0, targetHitpoints.hitpoints.Length)];
        aimTarget.target = target;
    }

    public static bool CheckForAimObstacles(AimTarget aimTarget, out RaycastHit hit, LayerMask aimMask)
    {
